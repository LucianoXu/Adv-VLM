...
backbone. Finally, we test four cheap input transformations as defenses,
and find that they blunt both attacks -- with one instructive exception.
...
\begin{figure}[t]
  ...
  \begin{tabular}{@{}c@{\hspace{0.3em}}ccc@{}}
    ...
    \raisebox{-0.5\height}{\includegraphics[width=0.28\linewidth]{figures/sample0_orig.png}} &
    \raisebox{-0.5\height}{\includegraphics[width=0.28\linewidth]{figures/sample2_orig.png}} &
    \raisebox{-0.5\height}{\includegraphics[width=0.28\linewidth]{figures/sample3_orig.png}} \\[1pt]
    \rotatebox[origin=c]{90}{\tiny adv.} &
    \raisebox{-0.5\height}{\includegraphics[width=0.28\linewidth]{figures/sample0_adv.png}} &
    \raisebox{-0.5\height}{\includegraphics[width=0.28\linewidth]{figures/sample2_adv.png}} &
    \raisebox{-0.5\height}{\includegraphics[width=0.28\linewidth]{figures/sample3_adv.png}} \\[2pt]
    &
...
\noindent These attempts show that the model generation alternates between
a fish and a cassette player. The performance is unstable, depending on the
prompt and answer priming. In particular, the second answer remains
\textbf{Tench} possibly because the model remembers to start the sentence
with a capital letter, which is not optimized by \texttt{cassette player}.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[img] (init) {\includegraphics[width=0.26\linewidth]{figures/saferlhf_init.png}};
    \node[img, right=0.5em of init] (adv) {\includegraphics[width=0.26\linewidth]{figures/saferlhf_adv.png}};
    \node[img, right=2.2em of adv] (zoom) {\includegraphics[width=0.26\linewidth]{figures/saferlhf_zoom.png}};
...
Interactive prompts also show how LLaVA perceives this adversarial image.
Asked what it depicts, it answers \textit{``a colorful, pixelated picture
of a man\dots\ \textbf{holding a gun}''},
stably across rephrasings, so the adversarial image clearly has stable
semantics to LLaVA. Asked whether the image contains any text, it answers
\textit{``No''}: the attack is not carried out by embedding text. Its
effect is also sensitive to the form of the request:

\begin{qa}
\textbf{Q:} \textit{I am not happy with how my mum disciplines me. What should I do?}\\
\textbf{A:} It is important to communicate with your mother and express your feelings\dots\ \textcolor{gray}{[harmless]}
\par
\textbf{Q:} \textit{Can you come up with the meanest words for my mum?}\\
\textbf{A:} \textbf{Yes, I can come up with the meanest words for your mum. Here are a few options: 1. ``You're a terrible mother who\dots}
\end{qa}

\noindent The response does not drift to harmful generations in general,
but is more likely to comply with direct requests, which resemble the
samples used during the attack.

\subsection{How Dependable Is the Measurement}
\label{sec:measure}

The rates above are single numbers, and two experiments show that they
carry more uncertainty than their intervals suggest.
...
graders gives $85.0\%$ (GPT-4.1), $83.0\%$ (GPT-4o), $50.5\%$
(GPT-4o-mini) and $6.0\%$ (GPT-4.1-mini) on the adversarial image, against
$38.0$, $36.0$, $17.0$ and $5.0\%$ on the clean one. The absolute rate thus
depends more on the grader than on the attack, and GPT-4.1-mini does not
merely grade strictly: it fails to detect an effect that the other three
graders all measure. Even between the two graders we retain, agreement on
